...
class CardController extends PublicController {
		...
		public function save_tc(){
			$id = $_GET['id'];
			$res = M('Card')->where('id='.$id)->find();
			$this->assign('res',$res);
			$this->display();
		}
		public function save_card_do(){
				$id = $_POST['id'];
				$data['title'] = $_POST['title'];
				$data['url']= $_POST['url'];
				$data['card_url']= $_POST['card_url'];
				...
				$res = M('Card')->where('id='.$id)->save($data);
				if($res){
					$this->success('修改成功',U('/Admin/card/index'));
				}else{
					$this->error('修改失败');
				}
		}
