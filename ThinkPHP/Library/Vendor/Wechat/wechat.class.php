...
class Wechat
{
	...
	const PAY_ORDERQUERY = 'https://api.mch.weixin.qq.com/pay/orderquery';
...
<Nonce><![CDATA[%s]]></Nonce>
	 ...
	 * @return string content
	 */
	private function http_post_bak($url,$param,$post_file=false){
		$oCurl = curl_init();
		if(stripos($url,"https://")!==FALSE){
			curl_setopt($oCurl, CURLOPT_SSL_VERIFYPEER, FALSE);
			curl_setopt($oCurl, CURLOPT_SSL_VERIFYHOST, false);
	...
	*/
		...
		$postdata['sign']=$this->getPaySignature($postdata);
		$xml=$this->xml_encode($postdata);
		$result = $this->http_post(self::PAY_ORDERQUERY,$xml,true);
		if ($result)
		{
